...
<?php
	...
	$consulta = "SELECT idEditora, editora, url FROM editoras WHERE idEditora=" .$idEditora;
	$resultado = mysqli_query($ligacao, $consulta);
	
	if($resultado){
		//existe o registo
		while($linha = mysqli_fetch_array($resultado)){
			$campo1= $linha["idEditora"];
			$campo2 = $linha["editora"];
			$campo3 = $linha["url"];
		}
		?>
			<p><?php echo $botao. " registo de editoras livreiras." ?></p>
			<div class="clear"></div>
			<form id="form_registo" method="POST" 
			action="processarRegistoEditor.php?id=<?php echo $campo1 ?>&mode=<?php echo $modo ?>">
				<div>
				...
				<div>
					...
					<?php 
						if ($modo=='edit') {
							echo "<input type='text' class='textbox' name='editoraNome' value='$campo2'>";
						}
						else {
							$botao = "Eliminar";
							echo "<input type='text' class='inactive2' name='editoraNome' value='$campo2' readonly>";
						}
					?>
					</span>
				</div>
				<div>
					...
					<?php 
						if ($modo=='edit') {
							echo "<input type='text' class='textbox' name='editoraUrl' value='$campo3'>";
						}
						else {
							$botao = "Eliminar";
							echo "<input type='text' class='inactive2' name='editoraUrl' value='$campo3' readonly>";
						}
					?>
					</span>
				</div>
