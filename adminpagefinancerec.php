...
<?php
	...
	$consulta = "SELECT idOrigem, origem FROM origem WHERE idOrigem=" .$idOrigem;
	$resultado = mysqli_query($ligacao, $consulta);
	
	if($resultado){
		//existe o registo
		while($linha = mysqli_fetch_array($resultado)){
			$campo1= $linha["idOrigem"];
			$campo2 = $linha["origem"];
		}
		?>
			<p><?php echo $botao. " registo de origem / medidas de financiamento." ?></p>
			<div class="clear"></div>
			<form id="form_registo" method="POST" 
			action="processarRegistoFinance.php?id=<?php echo $campo1 ?>&mode=<?php echo $modo ?>">
				<div>
					<span><label>ID:</label></span>
					<span><input type="text" class="short inactive" value="<?php echo $campo1 ?>" readonly></span>
				</div>
				<div>
					<span><label>Origem / Medida:</label></span>
					<span>
					<?php 
						if ($modo=='edit') {
							echo "<input type='text' class='textbox' name='origemNome' value='$campo2'>";
						}
						else {
							$botao = "Eliminar";
							echo "<input type='text' class='inactive2' name='origemNome' value='$campo2' readonly>";
						}
					?>
					</span>
				</div>
